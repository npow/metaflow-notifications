...
from tests.conftest import make_step_decorator, make_flow
...
def test_no_urls_emits_warning(recwarn, monkeypatch):
    ...
    assert any("no notifier" in str(w.message).lower() or "no url" in str(w.message).lower()
...
def test_dispatch_timeout_emits_warning(recwarn):
...
def test_foreach_warning_emitted_for_foreach_task(recwarn, monkeypatch):
    ...
    import metaflow_extensions.notifications.plugins.notify_decorator as mod
    mod._foreach_warned.clear()
...
def test_foreach_warning_emitted_only_once(recwarn, monkeypatch):
    ...
    foreach_warns = [w for w in recwarn.list if "foreach" in str(w.message).lower()
                     or "n notification" in str(w.message).lower()]
...
def test_injected_decorator_has_safe_run_id():
    from metaflow_extensions.notifications.plugins.notify_decorator import NotifyStepDecorator
